...
class BeaconScanner {
  ...
  void _processScanResult(ScanResult result) {
    ...
    debugPrint(
      ...
      ' | Manufacturer keys: ${ad.manufacturerData.keys.map((k) => k.toRadixString(16)).toList()}'
    );
    ...
    for (final entry in ad.manufacturerData.entries) {
      ...
      debugPrint('[BeaconScanner]   -> iBeacon detected! UUID=$uuid Major=$major Minor=$minor RSSI=${result.rssi}');

      final key = '${result.device.remoteId.str}_$uuid';
      final previous = _detectedBeacons[key];

      if (previous == null || (previous.rssi - result.rssi).abs() > 1) {
